...
def calculate_env(x_len: int, y_len: int, median_text_len: int) -> int:
    if x_len <= median_text_len:
        if y_len <= median_text_len:
            env = 0
        else:
            env = 1
    else:
...
def calculate_environments(x_attention_mask: torch.LongTensor, y_attention_mask: torch.LongTensor,
                           median_text_len: int) -> torch.LongTensor:
    ...
    environments = torch.tensor(
        data=[calculate_env(x_len[idx], y_len[idx], median_text_len) for idx in range(batchsize)],
        dtype=torch.long
    )
